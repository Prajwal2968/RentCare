...
const stripePromise = loadStripe("pk_test_51R1suwP2pQ6bNloojbUe76BeunvWqgOXuukkLMm3A5yIVkD5WWp3A1De4Nyi0xqM6LZ7VtDNuGFW42ZFQCcN8fvR00GP5zpLta");
...
function TenantDashboard() {
  ...
  const [isActionLoading, setIsActionLoading] = useState(false); // For specific button actions

  // useEffect for initial load animations
  useEffect(() => {
  ...
  }, [loading, tenant]); // Re-run if loading or tenant data changes
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:5001/properties/${propertyId}`)
      .then((res) => {
        if (!res.ok) {
  ...
  }, [propertyId, flatNo]); // Removed navigate from deps, it's stable
  ...
  const handlePayRent = async () => {
    ...
    try {
      ...
      // If successful, Stripe redirects. No need to clear message here.
    } catch (error) {
  ...
  const handleRaiseRequest = async () => {
    ...
    try {
        const res = await fetch(`http://localhost:5001/properties/${property.id}`, {
        ...
        if(updatedTenant) setTenant(updatedTenant); // Refresh tenant data if backend modifies it
  ...
  // Delete functions remain similar, just ensure setIsActionLoading is used
  ...
  if (loading) return (
    <div className="tenant-dashboard-page-wrapper">
        <div className="loading-container tenant-animate-on-load is-visible"> {/* Ensure spinner is visible */}
  ...
  const tenantMaintenanceRequests = (property.maintenanceRequests || []).filter(req => req.flatNo === tenant.flatNo);
  const tenantPaymentHistory = tenant.paymentHistory || [];
  const ownerNotifications = tenant.notifiedMessages || [];
      ...
      <div className="tenant-dashboard-container tenant-animate-on-load" data-delay="0">
        <header className="tenant-dashboard-header tenant-animate-on-load" data-delay="50">
            ...
            <h2 className="dashboard-title">My Dashboard</h2>
             {/* Optional: Logout button if needed here */}
        </header>
        {message && <p className={`dashboard-message ${message.toLowerCase().includes('error') || message.toLowerCase().includes('failed') ? 'error' : 'success'} tenant-animate-on-load`} data-delay="100">{message}</p>}
        ...
        <div className="dashboard-columns-wrapper tenant-animate-on-load" data-delay="250">
            <section className="dashboard-column maintenance-column">
                ...
                {tenantMaintenanceRequests.length > 0 ? (
                    ...
                    <div key={req.id} className="info-card maintenance-card">
            ...
            <section className="dashboard-column payment-column">
                ...
                <ul className="info-list">
                    ...
                    <li key={entry.id} className="info-card payment-card">
        ...
        <section className="notifications-section tenant-animate-on-load" data-delay="300">
            ...
            <ul className="info-list">
                ...
                <li key={msg.id} className="info-card notification-card">
